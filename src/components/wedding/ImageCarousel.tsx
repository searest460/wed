...
export const ImageCarousel = () => {
  const ImageList = () => (
    <div className="flex shrink-0">
      {images.map((src, index) => (
        <div 
          key={index} 
          className="flex-shrink-0 w-64 h-80 md:w-80 md:h-96 rounded-lg overflow-hidden shadow-md mx-2"
        >
          <img 
            src={src} 
            alt={`Gallery ${index + 1}`} 
  ...
  return (
    <div className="w-full overflow-hidden bg-white pb-16 md:pb-20">
      <div className="flex w-max animate-marquee hover:[animation-play-state:paused] transform-gpu py-4">
        <ImageList />
        <ImageList />
      </div>
    </div>
